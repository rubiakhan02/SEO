...
function isCaptchaOrBlockedPage() {
...
function extractResultsFromDocument(maxResults = 10) {
    const seen = new Set();
    const collected = [];
...
chrome.runtime.onMessage.addListener((message) => {
    if (!message || message.type !== "PARSE_GOOGLE_SERP") {
        return;
    }
    ...
    const pageStart = typeof message.pageStart === "number"
        ? message.pageStart
        : 0;
    const maxResults = typeof message.maxResults === "number"
        ? message.maxResults
        : 10;
